...
# Categories list
input_folder = "source-files/2024/cl-madrid/ai-results/" + phase + "/single"
output_file = 'source-files/2024/cl-madrid/ai-results/combined_' + phase + '_single.csv'
        ...
        for filename in os.listdir(input_folder):
            if filename.endswith('.csv') and filename != output_file:
                ...
                with open(file_path, 'r', encoding='utf-8') as infile:
                    ...
                    for row in reader:
                        # Strip spaces from the values in the row
                        cleaned_row = [value.strip() for value in row]

                        # Align the cleaned row to the final_header order
                        new_row = {
                            'category': filename.replace('ai_processed_', '').replace('.csv', ''),  # Extract category from filename
                            'name': cleaned_row[0],       # Assuming 'names' is the first column in the input files
                            'couple_number': cleaned_row[1],  # Assuming 'couple_number' is the second column
                            'judge_1': cleaned_row[2],     # Assuming the judges' scores follow
                            'judge_2': cleaned_row[3],
                            'judge_3': cleaned_row[4],
                            'judge_4': cleaned_row[5],
                            'judge_5': cleaned_row[6],
                            'total_score': cleaned_row[7],  # Assuming 'total_score' comes after the judges
                            'qualified': cleaned_row[8],   # Assuming 'qualified' is the last column
                        }
...
output_file = 'source-files/2024/cl-madrid/ai-results/combined_' + phase + '_withids_single.csv'  # Output file to save the result
...
# Categories list
input_folder = "source-files/2024/cl-madrid/ai-results/" + phase + "/couples"
output_file = 'source-files/2024/cl-madrid/ai-results/combined_' + phase + '_couples.csv'
final_header = ['category','names','couple_number','judge_1','judge_2','judge_3','judge_4','judge_5','total_score','qualified']  # Add your desired columns here
            ...
            if filename.endswith('.csv') and filename != output_file:
                ...
                with open(file_path, 'r', encoding='utf-8') as infile:
                    ...
                    for row in reader:
                        ...
                        new_row = {
                            'category': filename.replace('ai_processed_', '').replace('.csv', ''),  # Extract category from filename
                            'names': cleaned_row[0],       # Assuming 'names' is the first column in the input files
                            'couple_number': cleaned_row[1],  # Assuming 'couple_number' is the second column
                            'judge_1': cleaned_row[2],     # Assuming the judges' scores follow
                            'judge_2': cleaned_row[3],
                            'judge_3': cleaned_row[4],
...
def substitute_names_with_ids(results_file, name_to_id_mapping, output_file):
    with open(results_file, 'r', encoding='utf-8') as file:
        reader = csv.DictReader(file)
        fieldnames = ['category','couple_id','couple_number','judge_1','judge_2','judge_3','judge_4','judge_5','total_score','qualified']  # Get the original fieldnames
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as outfile:
            ...
            for row in reader:
                # Replace couple_name_1 and couple_name_2 with their corresponding ids
                row['couple_id'] = name_to_id_mapping.get(row['names'], row['names'])  # Keep name if no id is found
                

                writer.writerow({
                    'category': row['category'],
                    'couple_id': row['couple_id'],  # Use the new column names for the output
                    'couple_number': row['couple_number'],
                    'judge_1': row['judge_1'],
                    'judge_2': row['judge_2'],
                    'judge_3': row['judge_3'],
...
# File paths
name_id_file = 'source-files/2024/cl-madrid/database_export/couples.csv'  # CSV with id and names columns
results_file = output_file  # CSV with couple_name_1 and couple_name_2
output_file = 'source-files/2024/cl-madrid/ai-results/combined_' + phase + '_withids_couples.csv'  # Output file to save the result
